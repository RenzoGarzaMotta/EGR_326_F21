...
void intSysTickSetup(void){
    ...
    //Clear the STCVR register by writing to it with any value
    //Configure the STCSR register for the required operation.
    SysTick->CTRL = 0;                        //Turn off SysTick when setting up.
    SysTick->LOAD = (3000 * 2000) - 1;        //Load the timer the interval at which the US sensor is sampled
    SysTick->VAL = 0;                         //Clear on write
