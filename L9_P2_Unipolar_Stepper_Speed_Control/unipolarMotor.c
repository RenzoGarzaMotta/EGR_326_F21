...
void CCW(uint16_t steps){
    ...
    for(i = 0; i < steps; i++){
        UNI_PORT->OUT  &=~ UNI_MOTOR; //Turn off output
        UNI_PORT->OUT  |= (in_1[i%SEQ_SIZE] << 0 & INT_1); //Set output of A'
        UNI_PORT->OUT  |= (in_2[i%SEQ_SIZE] << 1 & INT_2);  //Set output of A
        UNI_PORT->OUT  |= (in_3[i%SEQ_SIZE] << 2 & INT_3); //Set output of B'
        UNI_PORT->OUT  |= (in_4[i%SEQ_SIZE] << 3 & INT_4); //Set output of B
        msDelay(speed);
    }
    UNI_PORT->OUT  &=~ UNI_MOTOR; //Turn off output
}
...
void CW(uint16_t steps){
    ...
    for(i = 0; i < steps; i++){
        UNI_PORT->OUT  &=~ UNI_MOTOR; //Turn off output
        UNI_PORT->OUT  |= (in_1[(SEQ_SIZE - 1) - (i%SEQ_SIZE)] << 0 & INT_1);
        UNI_PORT->OUT  |= (in_2[(SEQ_SIZE - 1) - (i%SEQ_SIZE)] << 1 & INT_2);
        UNI_PORT->OUT  |= (in_3[(SEQ_SIZE - 1) - (i%SEQ_SIZE)] << 2 & INT_3);
        UNI_PORT->OUT  |= (in_4[(SEQ_SIZE - 1) - (i%SEQ_SIZE)] << 3 & INT_4);
        msDelay(speed);
    }
    UNI_PORT->OUT  &=~ UNI_MOTOR; //Turn off output
}
